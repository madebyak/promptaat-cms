...
import { updateChangeLog } from '@/lib/data/changeLogs';
import { uploadMedia } from '@/lib/data/media';
import { ChangeLog } from '@/types/changeLog';

interface EditChangeLogModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (changelog: ChangeLog) => void;
  changeLogToEdit: ChangeLog | null;
}
...
export function EditChangeLogModal({ isOpen, onClose, onSuccess, changeLogToEdit }: EditChangeLogModalProps) {
  ...
  // Populate form when modal opens with change log data
  useEffect(() => {
    if (isOpen && changeLogToEdit) {
      setFormData({
        name: changeLogToEdit.name,
        image_url: changeLogToEdit.image_url || '',
        description: changeLogToEdit.description
      });
      setImagePreview(changeLogToEdit.image_url || null);
      setSelectedImageFile(null);
      setErrors({});
      setIsSubmitting(false);
    }
  }, [isOpen, changeLogToEdit]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      ...
      newErrors.name = 'Change log name must be less than 255 characters';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!changeLogToEdit || !validateForm()) {
      return;
    }
    ...
    try {
      ...
      if (selectedImageFile) {
        console.log('🚀 Edit Changelog: Uploading new image on save...');
        ...
        console.log('✅ Edit Changelog: Image uploaded successfully:', imageUrl);
      }

      const updatedChangeLog = await updateChangeLog(changeLogToEdit.id, {
      ...
      setErrors({ name: 'Failed to update change log. Please try again.' });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  if (!changeLogToEdit) return null;
      ...
      <DialogContent>
        <DialogHeader>
          <div className="flex items-center justify-between w-full">
            <div>
              <DialogTitle>Edit Change Log</DialogTitle>
              <DialogDescription>
                Update the change log information and description.
              </DialogDescription>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-0">
          ...
          <DialogFooter>
            <div className="flex flex-col-reverse sm:flex-row gap-3 w-full sm:w-auto">
              ...
              >
                {isSubmitting ? 'Updating...' : 'Update Change Log'}
              </Button>
            </div>
          </DialogFooter>
        </form>
