...
import { updatePrompt } from '@/lib/data/prompts';
...
export function EditPromptModal({ isOpen, onClose, onSuccess, categories, tools, prompt }: EditPromptModalProps) {
  ...
  // Initialize form with prompt data when modal opens
  useEffect(() => {
    if (isOpen && prompt) {
      // Extract category and tool IDs from the prompt data
      const categoryIds = prompt.categories?.map(cat => cat.category_id) || [];
      const subcategoryIds = prompt.categories?.map(cat => cat.subcategory_id).filter(id => id !== null && id !== undefined) as string[] || [];
      const toolIds = prompt.tools?.map(tool => tool.tool_id) || [];
      ...
      setFormData({
        ...
        visibility: prompt.visibility || 'published',
  ...
  const handleSubmit = async (e: React.FormEvent, isDraft: boolean = false) => {
    ...
    try {
      const updateData: Partial<CreatePromptData> = {
        name: formData.name,
        ...
        <DialogHeader>
          <div className="flex items-center justify-between w-full">
            <div>
              ...
              <DialogDescription>
                Update prompt information and assignments.
              </DialogDescription>
            </div>
